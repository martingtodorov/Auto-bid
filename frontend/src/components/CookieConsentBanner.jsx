...
export default function CookieConsentBanner() {
  ...
  return (
    <div
      className="fixed bottom-0 left-0 right-0 z-[60] bg-[hsl(var(--ink))] text-white px-4 py-4 sm:px-6 shadow-2xl border-t border-[hsl(var(--accent))] cookie-banner-root"
    ...
    >
      <div className="max-w-[1200px] mx-auto flex items-start gap-4 flex-wrap">
        ...
        <div className="flex-1 min-w-[240px] text-sm leading-relaxed">
          ...
          <p className="text-white/80 text-xs cookie-banner-body">
            {t("cookies.body")}{" "}
            <a href="/privacy" className="underline text-[hsl(var(--accent))]">{t("cookies.privacy_link")}</a>.
          </p>
        </div>
        <div className="flex items-center gap-2 ml-auto">
          <button onClick={() => set("rejected")} className="px-4 py-2 text-xs rounded-card border border-white/30 hover:bg-white/10 cookie-banner-reject" data-testid="cookie-reject">{t("cookies.reject")}</button>
          ...
          <button onClick={() => set("rejected")} aria-label="Close" className="p-1 hover:bg-white/10 rounded-full cookie-banner-close"><X size={16} /></button>
        </div>
      </div>
    </div>
  );
